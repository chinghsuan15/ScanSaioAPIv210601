...
public class ComManager {
    private final String TAG = "v2.ComManager";
    static
    {
    ...
    protected void broadcastDiagIntent(int ret)
    {
        Log.i(TAG, String.format("ComManager.%s() broadcast diag intent - return:0x%04X",
                Thread.currentThread().getStackTrace()[3].getMethodName(), ret & 0xFFFF));
        if (((ret & 0xF000) == 0xE000) || ret == ERR_OPERATION){
            long t = System.currentTimeMillis();

            Intent intent = new Intent();
            intent.setAction(SaioService.ACTION_DIAG_API);
            intent.putExtra(SaioService.EXTRA_DIAG_TYPE, SaioService.DIAG_TYPE_COMM);
            intent.putExtra(SaioService.EXTRA_DIAG_ERR, ret);
            intent.putExtra(SaioService.EXTRA_DIAG_TIMESTAMP, t);
            mContext.sendBroadcast(intent);

            Log.i(TAG, String.format("ComManager.%s() broadcast diag intent - error:0x%04X (t=%d)",
                    Thread.currentThread().getStackTrace()[3].getMethodName(), ret & 0xFFFF, t));
        }
    }
    ...
    public int[] getUsbDevId(int vid, int pid, int bcdDevice){
        ...
        int[] id_array = mNativeUsb.getUsbDevId(vid,pid,bcdDevice);
        mNativeUsb = null;
        return id_array;
    }
    ...
    public int open(int dev_id) {
        ...
        if (isUsbdevice) {
            ...
            if ((dev_id >= DEVICE_COM0 && dev_id <= DEVICE_COM4) || (dev_id >= DEVICE_EXT_USB0 && dev_id <= DEVICE_USB_ACCESSORY || dev_id == DEVICE_USB0)) {
                ret = mNativeSerialPort.open(dev_id);
            }else{
        ...
        if (isUsbdevice) {
            if (_rt != null) {
                try {
                    ...
                    Log.d(TAG, "close(rt): NullPointerException caught");
                }
                _rt = null;
            }
            ...
            if (mNativeUsb != null) {
                try {
                    ...
                    Log.d(TAG, "close(NativeUsb): NullPointerException caught");
                }
                mNativeUsb = null;
            }
            ret = 0;
    ...
    public int connect(int baud, int data_size, int stop_bit, int parity, int flow_control, int protocol, byte[] extra) {
        ...
        if (isUsbdevice) {
            ...
            ret = mNativeSerialPort.connect(baud, data_size, stop_bit, parity, flow_control, protocol, extra);
        }
        broadcastDiagIntent(ret);
        return ret;
    }
    ...
    public int connect(int protocol){
        ...
        if (isUsbdevice) {
            ...
            ret = connect(115200, 0, 0, 0, 0, protocol, null);
        }
        broadcastDiagIntent(ret);
        return ret;
    }
    ...
    public int read(byte[] data,int len,int timeout) {
        ...
        if (isUsbdevice) {
            if (mNativeUsb != null) {
                try {
                    ret = mNativeUsb.read(data, len, timeout);
                } catch(NullPointerException e) {
                    ret = ERR_NOT_OPEN;
                    Log.d(TAG, "read: NullPointerException caught");
                }
            }else{
                ret = ERR_NOT_OPEN;
            }
        }else{
            ret = mNativeSerialPort.read(data,len,timeout);
        }
        broadcastDiagIntent(ret);
        return ret;
    }
            ...
            if (mNativeUsb != null){
                try {
                    if(mNativeUsb.getProtocol() == PROTOCOL_XAC_VNG) {
                        ret = mNativeUsb.VNG_write(data,len,timeout);
                    }else if (mNativeUsb.getProtocol() == PROTOCOL_RAW_DATA){
                        ret = mNativeUsb.RAW_DATA_write(data,len,timeout);
                    }else {
                        ret = ERR_NO_CONNECTED;
                    }
                } catch(NullPointerException e) { ;
                    ret = ERR_NOT_OPEN;
                    Log.d(TAG, "write: NullPointerException caught");
                }
            }else{
                ret = ERR_NOT_OPEN;
            }
        }else {
            ret = mNativeSerialPort.write(data,len,timeout);
        }
        broadcastDiagIntent(ret);
        return ret;
    }
    ...
    public boolean isOpened() {
        if (mNativeUsb != null) {
            try {
                ...
                Log.d(TAG, "isOpened: NullPointerException caught");
                return false;
            }
    ...
    public int lastError(){
        ...
        if (isUsbdevice) {
            ...
            ret = mNativeSerialPort.lastError();
        }
        broadcastDiagIntent(ret);
        return ret;
    }
    ...
    public int setRts(int rts)
    {
        ...
        if (isUsbdevice) {
            ...
            ret = mNativeSerialPort.setRts(rts);
        }
        broadcastDiagIntent(ret);
        return ret;
    }
                ...
                if (mUsbReconnect && !isOpened()){
                    ...
                    if (mNativeUsb != null){
                        try {
                            ...
                            Log.d(TAG, "_ReadThread: NullPointerException caught");
                        }
                    }
                }
    ...
    private void checkDevice(int dev_id){
        if (dev_id > DEVICE_USB_ACCESSORY)
            isUsbdevice = true;
        else
            isUsbdevice = false;
    }
